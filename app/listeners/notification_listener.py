...
from app.events.mission_events import MeetingConfirmedByRunnerEvent, MeetingConfirmedByOrdererEvent
...
def on_meeting_confirmed_by_runner(event: MeetingConfirmedByRunnerEvent, db: Session) -> None:
    if _alarm_enabled(db, event.orderer_id):
        db.add(_pending(
            ...
            related_entity_id=event.mission_id,
            data={"mission_id": event.mission_id, "proposal_id": event.proposal_id},
        ))
