...
def test_list_missions_defaults_to_orderer_and_supports_runner_and_status_filter(client, db, sample_user):
    ...
    own_progress, _ = make_mission(db, sample_user, other_runner, MissionStatus.IN_PROGRESS)
    ...
    filtered_response = client.get("/v1/mission?role=ORDERER&status=IN_PROGRESS", headers=headers_for(sample_user))

    assert default_response.status_code == 200
    default_page = default_response.json()["data"]
    assert default_page["totalElements"] == 2
    assert {item["id"] for item in default_page["content"]} == {own_created.id, own_progress.id}
    ...
    assert filtered_page["content"][0]["id"] == own_progress.id
...
def test_runner_start_progress(client, db, sample_user):
    ...
    body = response.json()
    assert body["message"] == "미션 상태가 업데이트되었습니다."
    assert body["data"]["status"] == "IN_PROGRESS"
    assert body["data"]["pickupAt"] is not None

    db.refresh(mission)
    assert mission.status == MissionStatus.IN_PROGRESS
    assert mission.pickup_at is not None


def test_delivery_then_receipt_completes_mission_and_offer(client, db, sample_user):
    runner = make_user(db, "01088880005")
    mission, offer = make_mission(db, sample_user, runner, MissionStatus.IN_PROGRESS)
...
def test_receipt_then_delivery_completes_mission_and_offer(client, db, sample_user):
    runner = make_user(db, "01088880006")
    mission, offer = make_mission(db, sample_user, runner, MissionStatus.IN_PROGRESS)
...
def test_update_mission_validation_author_and_state_errors(client, db, sample_user):
    ...
    missing = client.put("/v1/mission/999999", json={"action": "START_PROGRESS"}, headers=headers_for(runner))
    ...
    proof_missing = client.put(
        f"/v1/mission/{mission.id}",
        json={"action": "COMPLETE_DELIVERY"},
        headers=headers_for(runner),
    )
    not_updatable = client.put(
        f"/v1/mission/{completed.id}",
        json={"action": "COMPLETE_DELIVERY", "proofImageUrl": "https://cdn.example/proof.jpg"},
        headers=headers_for(runner),
    )
    ...
    assert proof_missing.status_code == 400
    assert proof_missing.json()["error"]["code"] == "VALIDATION_ERROR"
...
def test_dispute_requires_reason_and_allows_mission_actor(client, db, sample_user):
    ...
    mission, _ = make_mission(db, sample_user, runner, MissionStatus.IN_PROGRESS)
    ...
    disputed = client.put(
        f"/v1/mission/{mission.id}",
        json={"action": "DISPUTE", "disputeReason": "물품이 다릅니다."},
        headers=headers_for(runner),
    )
