...
- `WAITING` -> `REJECTED`
- `WAITING` -> `CANCELLED`
- `ACCEPTED` -> `DELIVERY_COMPLETED`
- `DELIVERY_COMPLETED` -> `RECEIPT_CONFIRMED`
- `RECEIPT_CONFIRMED` -> `SETTLED`
- `ACCEPTED`/`DELIVERY_COMPLETED`/`RECEIPT_CONFIRMED` -> `DISPUTED`
- `DISPUTED` -> `REFUNDED`
...
CREATE TABLE offers (
    ...
    status ENUM('WAITING', 'ACCEPTED', 'DELIVERY_COMPLETED', 'RECEIPT_CONFIRMED', 'SETTLED', 'DISPUTED', 'REFUNDED', 'REJECTED', 'CANCELLED') NOT NULL DEFAULT 'WAITING',
