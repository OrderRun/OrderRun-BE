...
- `WAITING` -> `REJECTED`
- `WAITING` -> `CANCELLED`
- `ACCEPTED` -> `RUNNER_COMPLETED`
- `RUNNER_COMPLETED` -> `ALL_COMPLETED`
- `ACCEPTED`/`RUNNER_COMPLETED`/`ALL_COMPLETED` -> `DISPUTED`
- `DISPUTED` -> `REFUNDED`
...
CREATE TABLE offers (
    ...
    status ENUM('WAITING', 'ACCEPTED', 'RUNNER_COMPLETED', 'ALL_COMPLETED', 'DISPUTED', 'REFUNDED', 'REJECTED', 'CANCELLED') NOT NULL DEFAULT 'WAITING',
