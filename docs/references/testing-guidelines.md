...
- API 응답 필드, status code, 성공/실패 케이스가 바뀌면 OpenAPI 예시와 계약 테스트를 함께 갱신한다.
- 통합 테스트의 실제 응답은 문서화된 OpenAPI 예시 중 하나와 shape, `message`, `error.code`가 맞아야 한다.
...
4. OpenAPI에 노출되는 계약 변경이면 `tests/test_openapi.py`와 문서 예시를 함께 갱신한다. 성공/실패 분기가 추가되면 해당 케이스 예시도 추가한다.
