...
## `@dataclass(frozen=True)` 사례

`dataclass`는 클래스에 적용하는 Python 표준 라이브러리 데코레이터다. 선언된 필드를 바탕으로 `__init__`, `__repr__`, `__eq__` 같은 반복 코드를 자동 생성한다.
...
따라서 별도의 생성자를 작성하지 않아도 다음과 같이 사용할 수 있다.

```python
spec = ApiErrorSpec(404, "USER_NOT_FOUND", "User not found")
...
`frozen=True`는 인스턴스 생성 후 필드를 다른 값으로 재할당하지 못하게 한다.

```python
spec.code = "OTHER_ERROR"
# dataclasses.FrozenInstanceError
```

이 설정이 객체 전체의 깊은 불변성을 보장하는 것은 아니다. 필드가 리스트처럼 변경 가능한 객체를 참조하면 필드 자체를 교체할 수는 없지만, 그 객체의 내부는 변경할 수 있다.
...
example.values = ["B"]  # FrozenInstanceError: 필드 재할당
example.values.append("B")  # 가능: 기존 리스트 내부 변경
```

따라서 `ApiErrorSpec`은 필드 재할당을 막은 오류 명세 값 객체라고 이해할 수 있다. 현재 필드인 `int`와 `str`도 변경 불가능한 타입이므로 실제 사용에서는 불변 값처럼 안전하게 동작한다.
