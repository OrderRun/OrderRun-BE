...
- Mission은 생성 시점의 `ordererId`, `runnerId`, `runFee`, `itemPrice`, `totalAmount`를 스냅샷으로 저장하며 이후 변경하지 않는다.
...
- Mission에는 `proposalId`, `offerId`, `ordererId`, `runnerId`, `runFee`, `itemPrice`, `totalAmount`가 저장된다.
